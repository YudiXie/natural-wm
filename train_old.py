...
def model_train(config: BaseConfig):
    ...
    # initialize dataset
    train_data = DatasetIters(config, True, task_batch_s)
    if config.perform_test:
        test_data = DatasetIters(config, False, task_batch_s)
    else:
        test_data = None

    # initialize network
    net = model_init(config, train_data.datum_sizes, mode='train')
    ...
    if config.use_lr_scheduler:
        ...
        scheduler_step_after_eval = False
        ...
        if config.scheduler_type == 'ExponentialLR':
            ...
            # Run sched.step() after each evaluation
            scheduler_step_after_eval = True 
            assert config.perform_test
        else:
    ...
    for epoch in range(config.num_ep):
        ...
        for step_ in range(train_data.min_iter_len):
            ...
            if i_b % config.log_every == config.log_every - 1:
                ...
                if config.use_lr_scheduler and scheduler_step_after_eval:
                    scheduler.step()
                ...
                if config.early_stop\
                    ...
                    if config.use_lr_scheduler and not scheduler_step_after_eval:
                        scheduler.step()
