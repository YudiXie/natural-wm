...
class Logger:
    ...
    def log_tabular_dict(self, d):
        """
        Log a dictionary of diagnostic quantities.

        Equivalent to calling ``log_tabular`` on each key-value pair in the
        dictionary.
        """
        for key, val in d.items():
            self.log_tabular(key, val)

    def dump_tabular_dict(self, d):
        self.log_tabular_dict(d)
        self.dump_tabular()
